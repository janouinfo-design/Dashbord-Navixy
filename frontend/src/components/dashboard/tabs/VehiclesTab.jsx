...
import { getScoreBg, getScoreColor, calcFuelCost, FUEL_PRICE_CHF } from "@/lib/metrics";
import {
...
export const VehiclesTab = ({ data }) => {
  ...
  const vehicles = (stats?.vehicles || []).map(v => ({
    ...v, ...(compMap[v.tracker_id] || {}), efficiency_score: compMap[v.tracker_id]?.efficiency_score || v.efficiency || 0
  }));
  ...
  const ClickKPI = ({ label, value, unit, icon: Icon, color, subtitle, filterKey }) => {
    const isActive = activeKPI === filterKey;
    return (
      <div onClick={() => filterKey && handleKPI(filterKey)}
  ...
  return (
    <div className="p-4 lg:p-8 space-y-6 max-w-[1600px] mx-auto" data-testid="vehicles-tab">
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
        ...
        <ClickKPI label="Actifs" value={activeV} icon={Wifi} color="text-emerald-600" filterKey="active" />
        <ClickKPI label="Hors ligne" value={offlineV} icon={WifiOff} color={offlineV > 0 ? 'text-red-500' : 'text-gray-400'} filterKey="offline" />
        <ClickKPI label="Sous-utilises" value={underUsedList.length} icon={XCircle} color={underUsedList.length > 0 ? 'text-amber-600' : 'text-gray-400'} subtitle="< 10 km" filterKey="underused" />
        ...
        <MiniKPI label="Cout carburant" value={Math.round(totalKm * 8.5 / 100 * FUEL_PRICE_CHF).toLocaleString('fr-FR')} unit="CHF" icon={DollarSign} />
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <div className="relative"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" /><input type="text" placeholder="Rechercher..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 pr-3 py-1.5 text-xs bg-gray-50 border border-gray-200 rounded-lg focus:outline-none w-44" /></div>
            <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="text-xs bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none"><option value="all">Tous</option><option value="active">Actifs</option><option value="offline">Offline</option></select>
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead><tr className="border-b border-gray-100">
              {[{ k: 'label', l: 'Vehicule' }, { k: 'connection_status', l: 'Etat GPS' }, { k: 'mileage', l: 'Km' }, { k: 'engine_hours', l: 'Moteur' }, { k: 'fuel_efficiency', l: 'Conso.' }, { k: 'last_update', l: 'Dern. comm.' }, { k: 'efficiency_score', l: 'Score' }].map(col => (
            ...
            <tbody>
              {filtered.map(v => (
                <React.Fragment key={v.tracker_id}>
                  <tr className={`fleet-row border-b border-gray-50 cursor-pointer ${expandedVehicle === v.tracker_id ? 'bg-gray-50' : ''}`} onClick={() => setExpandedVehicle(expandedVehicle === v.tracker_id ? null : v.tracker_id)}>
                    ...
                    <td className="px-4 py-3"><span className={`text-xs font-medium ${(v.fuel_efficiency || 0) > 10 ? 'text-amber-600' : 'text-gray-700'}`}>{v.fuel_efficiency || '-'}</span></td>
                    ...
                    <td className="px-4 py-3"><div className="flex items-center gap-2"><div className="w-10 h-1.5 bg-gray-100 rounded-full overflow-hidden"><div className={`h-full rounded-full ${getScoreBg(v.efficiency_score)}`} style={{ width: `${Math.min(100, v.efficiency_score)}%` }} /></div><span className={`text-xs font-semibold ${getScoreColor(v.efficiency_score)}`}>{v.efficiency_score}%</span></div></td>
                  </tr>
                      ...
                      <div className="bg-gray-50 border-b border-gray-200 px-6 py-4">
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
                          {[
                            ...
                            { l: 'Cout', v: `${calcFuelCost(v)} CHF`, i: DollarSign, c: 'text-gray-500' },
                          ].map((item, idx) => (
